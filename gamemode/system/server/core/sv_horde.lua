...
function HORDE:OnEnemyKilled(victim, killer, weapon)
    ...
    if HORDE.spawned_enemies[victim:EntIndex()] then
        ...
        if (not HORDE.horde_boss) or (HORDE.horde_boss and (not horde_boss_properties) ) or (HORDE.horde_boss and horde_boss_properties and (not horde_boss_properties.unlimited_enemies_spawn)) then
...
function HORDE:SpawnEnemy(enemy, pos)
    ...
    if HORDE.hundred_mode and HORDE.current_wave > HORDE.max_max_waves then
        spawned_enemy:SetMaxHealth(spawned_enemy:GetMaxHealth() * (HORDE.endless_health_multiplier or 1))
    end
    ...
    if enemy.mutation and enemy.mutation ~= "" then
        ...
        -- HUNDRED: add cumulative mutation bonus (+0.25 every 5 waves after wave 10)
        if HORDE.hundred_mode and (HORDE._hundred_mutation_bonus or 0) > 0 then
            mut_prob = mut_prob + HORDE._hundred_mutation_bonus
        end
    ...
    for i = 0, math.random(HORDE.min_base_enemy_spawns_per_think + HORDE.difficulty_additional_pack[HORDE.difficulty] + math.floor(horde_players_count / 2), HORDE.max_base_enemy_spawns_per_think + HORDE.difficulty_additional_pack[HORDE.difficulty] + horde_players_count) do
        if (#enemies + 1 <= HORDE.max_enemies_alive) and (HORDE.total_enemies_this_wave > 0) then
            ...
            if pos ~= nil then
                ...
                local p_cum = 0
                local spawned_enemy
                -- For HUNDRED waves 11+, use the remapped wave index so enemy lookup matches
                local enemy_wave
                if HORDE.hundred_mode and HORDE.current_wave > HORDE.max_max_waves then
                    if HORDE.current_wave % 5 == 0 then
                        enemy_wave = (math.random(2) == 1) and 5 or 10
                    else
                        enemy_wave = ((HORDE.current_wave - 11) % 4) + 6
                    end
                else
                    enemy_wave = ((HORDE.current_wave - 1) % HORDE.max_max_waves) + 1
                end
                ...
                for name, orders in pairs(horde_force_spawn_enemies) do
                    for idx, order in pairs(orders) do
                        if HORDE.total_enemies_this_wave < order then
                            ...
                            if not enemy then goto cont3 end
...
function HORDE:SpawnBoss(enemies, valid_nodes)
    if (HORDE.horde_boss_name) and (#enemies + 1 <= HORDE.max_enemies_alive) and (not HORDE.horde_boss) and (HORDE.total_enemies_this_wave > 0) then
        ...
        local enemy_wave
        if HORDE.hundred_mode and HORDE.current_wave > HORDE.max_max_waves then
            -- Use the wave that was decided in WaveStart (not a new random roll)
            enemy_wave = HORDE._hundred_boss_wave or 5
        else
            enemy_wave = ((HORDE.current_wave - 1) % HORDE.max_max_waves) + 1
        end
        ...
        if not enemy then
            -- Fallback: try the other boss wave
            local alt_wave = (enemy_wave == 5) and 10 or 5
            enemy = HORDE.bosses[HORDE.horde_boss_name .. tostring(alt_wave)]
        end
        if not enemy then return end  -- boss data not found, skip spawn
        enemy.is_elite = true
        spawned_enemy = HORDE:SpawnEnemy(enemy, pos + Vector(0, 0, HORDE.enemy_spawn_z))
...
function HORDE:WaveStart()
    ...
    if HORDE.enemies_normalized[HORDE.current_wave] and table.IsEmpty(HORDE.enemies_normalized[HORDE.current_wave]) then
        HORDE:SendNotification("No enemy config set for this wave. Falling back to previous wave settings.", 1)
    end

    -- For HUNDRED waves 11-100: cycle enemy configs 6→9, boss every 5th wave
    local current_wave
    if HORDE.hundred_mode and HORDE.current_wave > HORDE.max_max_waves then
        if HORDE.current_wave % 5 == 0 then
            -- Every 5th wave: use boss wave (5 or 10, random)
            current_wave = (math.random(2) == 1) and 5 or 10
        else
            -- Cycle through waves 6-9
            current_wave = ((HORDE.current_wave - 11) % 4) + 6
        end
    else
        current_wave = ((HORDE.current_wave - 1) % HORDE.max_max_waves) + 1
    end
    horde_players_count = table.Count(player.GetAll())
    -- Fallback: if remapped wave has no enemies, use wave 9
    local _enemy_src = HORDE.enemies_normalized[current_wave]
    if not _enemy_src or table.IsEmpty(_enemy_src) then
        _enemy_src = HORDE.enemies_normalized[9] or HORDE.enemies_normalized[HORDE.max_max_waves] or {}
    end
    horde_current_enemies_list = table.Copy(_enemy_src)
    local difficulty_coefficient = HORDE.difficulty * 0.05

    if HORDE.hundred_mode and HORDE.current_wave > HORDE.max_max_waves then
        -- HUNDRED: waves 11-100 use wave 10 enemy list, scaled up each wave
        local over = HORDE.current_wave - HORDE.max_max_waves
        HORDE.total_enemies_this_wave = (HORDE.total_enemies_per_wave[HORDE.max_max_waves] + 5 * over) *
            math.ceil(horde_players_count * (0.75 + difficulty_coefficient))
        -- HP/DMG: +2.5% per wave after wave 10
        local scale_per_wave = HORDE.hundred_scale_per_wave or 0.025
        local hp_dmg_scale = math.min(1 + scale_per_wave * over, 1.5)  -- cap at +50%
        HORDE.endless_health_multiplier = hp_dmg_scale
        HORDE.endless_damage_multiplier = hp_dmg_scale

        -- Mutation chance: +0.25 every 5th wave, capped at +2.5
        local mutation_bonus = math.min(math.floor(over / 5) * 0.25, 2.5)
        HORDE._hundred_mutation_bonus = mutation_bonus
    else
        -- Normal mode (endless removed)
    ...
    -- For HUNDRED: boss waves (every 5th) use 5 or 10; save so SpawnBoss uses same wave
    if HORDE.hundred_mode and HORDE.current_wave > HORDE.max_max_waves then
        if HORDE.current_wave % 5 == 0 then
            current_wave = (math.random(2) == 1) and 5 or 10
            HORDE._hundred_boss_wave = current_wave  -- store for SpawnBoss
        else
            HORDE._hundred_boss_wave = nil
        end
    end
    ...
    -- For HUNDRED waves 11+, use the same remapped wave index for forced spawns
    local enemy_wave = current_wave
    ...
    -- Skip for HUNDRED waves beyond 10 — objectives were already done in waves 1-10
    if not has_boss and not (HORDE.hundred_mode and HORDE.current_wave > HORDE.max_max_waves) then
...
function HORDE:WaveEnd()
    ...
    if (HORDE.current_wave >= HORDE.max_waves) and (true) then
        ...
        if HORDE.hundred_mode and HORDE.current_wave >= HORDE.max_max_waves then
            -- HUNDRED: show progress towards wave 100
            local waves_left = HORDE.max_waves - HORDE.current_wave
            HORDE:SendNotification("Wave " .. HORDE.current_wave .. "/" .. HORDE.max_waves .. " complete! " .. waves_left .. " to go!", 0)
        else
    ...
    for _, ply in pairs(player.GetAll()) do
        ...
        net.Start("Horde_PlayerReadySync")
            local _ready_sync = {}
    for _ply, _status in pairs(HORDE.player_ready) do
        if IsValid(_ply) then _ready_sync[_ply:SteamID()] = _status end
    end
    net.WriteTable(_ready_sync)
        net.Broadcast()
