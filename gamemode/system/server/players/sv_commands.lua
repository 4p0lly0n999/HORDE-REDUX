...
function HORDE:BroadcastBreakCountDownMessage(wave, is_end_message)
    net.Start("Horde_RenderBreakCountDown")
        net.WriteInt(math.max(0, wave), 8)  -- never send negative values
...
concommand.Add("horde_testing_wave_goto", function (ply, cmd, args)
    ...
    local target_wave = tonumber(args[1])
    if not target_wave or target_wave < 1 then
        HORDE:SendNotification("Usage: horde_testing_wave_goto <wave number>", 1, ply)
        return
    end
    target_wave = math.floor(math.min(target_wave, HORDE.max_waves))

    -- Просто заканчиваем текущую волну и отправляем в break перед нужной волной.
    -- WaveEnd() и StartBreak() — уже проверенный путь, ничего не ломает.

    -- Устанавливаем номер волны: break отсчитает и WaveStart поставит current_wave+1,
    -- поэтому ставим target - 1
    HORDE.current_wave = target_wave - 1

    -- Принудительно завершить текущую волну через штатный путь
    HORDE:WaveEnd()
    hook.Run("HordeWaveEnd", HORDE.current_wave)

    HORDE:SendNotification("Going to wave " .. target_wave .. " after break...", 0, ply)
end)
