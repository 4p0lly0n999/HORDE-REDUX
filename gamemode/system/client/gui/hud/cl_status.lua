...
hook.Add("HUDPaint", "Horde_DrawHud", function ()
    ...
    if GetConVarNumber("horde_enable_health_gui") == 1 then
        ...
        -- Draw Class
        local subclass = HORDE.subclasses and HORDE.subclasses[MySelf:Horde_GetCurrentSubclass()]
        if not subclass then subclass = HORDE.subclasses and HORDE.subclasses[HORDE.Class_Survivor] end
        if not subclass then return end  -- subclasses not loaded yet
