...
net.Receive("Horde_RenderEnemiesCount", function()
    ...
    local _dn = HORDE.difficulty_text and HORDE.difficulty_text[HORDE.difficulty] or ""
    local _dl = (_dn ~= "") and (translate.Get("Game_Difficulty_" .. _dn) or _dn) or ""
    if is_boss then
        center_panel_str = "|" .. _dl .. "|  BOSS"
    else
        center_panel_str = "|" .. _dl .. "|  " .. translate.Get("Game_Enemies") .. ": " .. tostring(count)
    end
end)
...
net.Receive("Horde_RenderObjectives", function()
    ...
    local _dn2 = HORDE.difficulty_text and HORDE.difficulty_text[HORDE.difficulty] or ""
    local _dl2 = (_dn2 ~= "") and (translate.Get("Game_Difficulty_" .. _dn2) or _dn2) or ""
    center_panel_str = "|" .. _dl2 .. "|  Objectives: " .. tonumber(count) .. "/" .. tonumber(max_count)
end)
...
net.Receive("Horde_SyncEscapeStart", function ()
    local _dn3 = HORDE.difficulty_text and HORDE.difficulty_text[HORDE.difficulty] or ""
    local _dl3 = (_dn3 ~= "") and (translate.Get("Game_Difficulty_" .. _dn3) or _dn3) or ""
    center_panel_str = "|" .. _dl3 .. "|  Escape!"
end)
