...
function PANEL:SetData(status, mvp_player, mvp_damage, mvp_kills, damage_player, most_damage, kills_player, most_kills, most_heal_player, most_heal, headshot_player, most_headshots, elite_kill_player, most_elite_kills, damage_taken_player, most_damage_taken, total_damage, maps)
    ...
    summary_label.Paint = function ()
        ...
        local _dl = (_dn ~= "") and translate.Get("Game_Difficulty_" .. _dn) or ""
        draw.SimpleText(translate.Get("Game_Result_" .. status) .. "! " .. game.GetMap() .. " - " .. _dl, "LargeTitle", 450, 0, Color(255,255,255), TEXT_ALIGN_CENTER)
    end

    -- Dynamic: reads from HORDE.difficulty_text which is populated by modules/difficulties/
    for _, diff_name in ipairs(HORDE.difficulty_text) do
        self.create_diff_panel(diff_name)
    end
end
